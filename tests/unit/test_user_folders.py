...
async def test_ensure_user_folder_keeps_matching_folder() -> None:
    u = user(root_folder="disk:/Root/123_user/", allowed_folders=["disk:/Root/123_user/"])
    session = FakeSession()
    client = FakeClient()
    ...
    assert folder == "disk:/Root/123_user/"
    assert u.allowed_folders == ["disk:/Root/123_user/"]
    assert client.calls == ["disk:/Root/123_user/"]
...
async def test_ensure_user_folder_updates_when_root_changed() -> None:
    ...
    assert folder == "disk:/New/123_user/"
    assert u.root_folder == "disk:/New/123_user/"
    assert u.allowed_folders == ["disk:/New/123_user/"]
...
async def test_change_root_updates_active_users_and_not_upload_requests() -> None:
    u = user(root_folder="disk:/Old/123_user/", allowed_folders=["disk:/Old/123_user/"])
    upload = UploadRequest(
        target_folder="disk:/Old/123_user/", target_path="disk:/Old/123_user/a.txt"
    )
    ...
    assert client.calls == ["disk:/New", "disk:/New/123_user/"]
    assert u.root_folder == "disk:/New/123_user/"
    assert u.allowed_folders == ["disk:/New/123_user/"]
    assert upload.target_folder == "disk:/Old/123_user/"
