...
- Администратор может открыть временный файл, посмотреть содержимое целевой папки, одобрить загрузку, загрузить как копию, перезаписать, повторить failed-загрузку или отклонить заявку. Команды `/diskroot` и `/setdiskroot` показывают и меняют корневую папку для новых пользователей.
...
- `/setdiskroot` меняет root только для будущих approve. Уже одобренные пользователи остаются в своих текущих `user.root_folder`.
...
- Upload flow продолжает использовать назначенный `user.root_folder`, а не глобальный root.
